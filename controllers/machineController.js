...
module.exports = {
    ...
    readDB: async(req, res) => {
        try {
            ...
            let q = `SELECT uuid as mc_param_id , machine_id,line_id, line_nm, machine_nm, x_axis, y_axis, tag_name, reg_value as status FROM v_mc_param_status ${whereCond.length > 0 ? 'WHERE ' + whereCond : ''}`
            console.log(q);
